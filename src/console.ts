...
const MAX_LOGS = 200;
export const consoleLogs: Array<{ level: string; args: unknown[]; ts: number }> = [];
...
export function consoleLogTool(): McpTool {
  return {
    ...
    description: 'Returns browser console logs captured from the running page (requires /mcp/console.js).',
    inputSchema: {
      type: 'object',
      properties: {
