...
export function defaultTools(ctx: ManicServerPluginContext): McpTool[] {
  ...
  const tools: McpTool[] = [
    {
      name: 'get_routes',
      description: 'Returns all page routes registered in the Manic app (path + file path).',
      inputSchema: { type: 'object' },
      execute: () => ctx.pageRoutes,
    },
    {
      name: 'get_api_routes',
      description: 'Returns all API routes registered under /api in the Manic app.',
      inputSchema: { type: 'object' },
      execute: () => ctx.apiRoutes,
    },
    {
      name: 'get_page_meta',
      description: 'Fetches a page and extracts its <title>, meta tags (og:*, description, etc.), and canonical link.',
      inputSchema: {
        type: 'object',
      ...
      async execute({ url }) {
        ...
        const res = await fetch(target.startsWith('http') ? target : `http://localhost:${port}${target}`);
        const html = await res.text();
        const title = html.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1] ?? null;
        const metas: Record<string, string> = {};
        for (const m of html.matchAll(/<meta\s+([^>]+)>/gi)) {
          const name = m[1].match(/(?:name|property)=["']([^"']+)["']/i)?.[1] ?? null;
    ...
    },
    {
      ...
      description: 'Fetches a page and returns a simplified element list (tag, id, class, text) for AI inspection.',
      inputSchema: {
        type: 'object',
        properties: {
